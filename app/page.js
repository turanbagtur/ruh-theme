      ...
      <section className="page-container page-section pt-0 pb-40">
        <div className="main-layout">
          ...
          <aside className="sidebar">
            ...
            {/* Most Read (En Çok Okunanlar) Segmented List */}
            <div className="sidebar-widget mr-widget">
              <div className="widget-header sticky-header">
                <h3>Most Read</h3>
              </div>
              <div className="ranking-tabs-small">
                <button className={topPeriod === 'daily' ? 'active' : ''} onClick={() => setTopPeriod('daily')}>Daily</button>
                <button className={topPeriod === 'weekly' ? 'active' : ''} onClick={() => setTopPeriod('weekly')}>Weekly</button>
                <button className={topPeriod === 'monthly' ? 'active' : ''} onClick={() => setTopPeriod('monthly')}>Monthly</button>
                <button className={topPeriod === 'all' ? 'active' : ''} onClick={() => setTopPeriod('all')}>All</button>
              </div>
              
              <div className="sidebar-list mt-3" style={{ opacity: topLoading ? 0.3 : 1, transition: 'opacity 0.2s ease-in-out', pointerEvents: topLoading ? 'none' : 'auto', minHeight: '300px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                {topSeries.map((s, idx) => (
                  <Link key={s.id} href={`/series/${s.slug || s.id}`} className="asura-sidebar-item">
                    <div className="asura-sidebar-cover" style={{ overflow: 'visible' }}>
                      <div className="asura-rank position-absolute">{idx + 1}</div>
                      <img src={s.cover_url || '/demo/cover1.jpg'} alt="" loading="lazy" style={{ borderRadius: 'var(--radius)', width: '100%', height: '100%', objectFit: 'cover' }} />
                    </div>
                    <div className="asura-sidebar-info">
                      <span className="asura-sidebar-title">{s.title}</span>
                      <span className="asura-sidebar-meta">
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="var(--accent-light)" stroke="none" strokeWidth="2"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" /></svg>
                        {s.rating?.toFixed(1) || '0.0'} &middot; {topPeriod === 'all' ? (s.views || 0).toLocaleString() : (s.period_views || 0).toLocaleString()} views
                      </span>
                      <div style={{ display: 'flex', gap: '4px', marginTop: '2px' }}>
                        {parseGenres(s.genres).slice(0, 2).map((g, i) => (
                          <span key={i} style={{ fontSize: '0.65rem', padding: '2px 6px', background: 'rgba(255,255,255,0.05)', borderRadius: '2px', color: 'var(--text-secondary)' }}>{g}</span>
                        ))}
                      </div>
                    </div>
                  </Link>
                ...
                {!topLoading && topSeries.length === 0 && (
                  <div className="empty-text">No views recorded.</div>
                )}
              </div>
            </div>
