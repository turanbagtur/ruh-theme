 ...
 * 
 * Canvas tabanlı kırpma sistemi:
 * Client-side'da canvas kullanılarak görsel kırpılır ve zaten kırpılmış görsel gönderilir.
 * Backend'de bu görsel sadece boyutlandırılır ve WebP'ye dönüştürülür.
 * 
 * cropApplied=true ise:
 *   - Görsel zaten client-side'da canvas ile kırpılmıştır
 *   - Sadece hedef boyutlara getirilir (200x200)
 * 
 * cropApplied=false ise:
 *   - Eski davranış: auto-center crop
 */
export async function optimizeAvatar(inputBuffer, outputPath, cropOptions = {}) {
    const webpPath = outputPath.replace(/\.(jpg|jpeg|png|gif|webp)$/i, '.webp');
    ...
    // Canvas tabanlı kırpma yapıldıysa (cropApplied=true)
    // Görsel zaten client-side'da kırpılmıştır, sadece boyutlandır
    if (cropApplied) {
        if (await isWebPAlreadyOptimized(inputBuffer, 200, 200)) {
            fs.writeFileSync(webpPath, inputBuffer);
            return webpPath;
        }
        // Görseli 200x200 boyutuna getir (fit:'cover' ile kırp)
        await sharp(inputBuffer)
    ...
    // Kırpma yoksa eski davranış (kullanıcı crop dialog'unu onaylamadı)
    ...
    // 200×200'ü kaplayacak temel ölçek faktörü (fit:'cover' ile aynı mantık)
    const sf = Math.max(200 / origW, 200 / origH);

    // Kullanıcının zoom'u en az 1 olmalı
    ...
    // cropX/Y: kullanıcı sürükleme ofseti. Önizlemede image-space koordinatları
    // (drag delta / scale şeklinde hesaplanmış). Çıktıya direkt uygulanır.
 ...
 * cropApplied=true ise:
 *   - Görsel zaten client-side'da canvas ile kırpılmıştır
 *   - Sadece hedef boyutlara getirilir (1200x400)
 * 
 * cropApplied=false ise:
 *   - Eski davranış: auto-center crop
 */
export async function optimizeProfileCover(inputBuffer, outputPath, cropOptions = {}) {
    ...
    if (cropApplied) {
        ...
        // Görseli 1200x400 boyutuna getir (fit:'cover' ile kırp)
        await sharp(inputBuffer)
    ...
    // cropX/Y: kullanıcı sürükleme ofseti
