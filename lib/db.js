...
export function getDb() {
  if (!db) {
    ...
    db.pragma('cache_size = -64000');      // 64MB sayfa önbelleği
    db.pragma('mmap_size = 268435456');    // 256MB memory-mapped I/O
    db.pragma('synchronous = NORMAL');     // WAL modda FULL'dan daha hızlı, veri güvenli
    db.pragma('temp_store = MEMORY');      // Geçici tablolar RAM'de
    db.pragma('busy_timeout = 5000');      // Kilit bekleme süresi 5sn
...
function migrateDatabase(db) {
  try {
    ...
    try {
      db.prepare("DELETE FROM quest_progress WHERE quest_date < date('now', '-30 days')").run();
    } catch {}

    // chapter_views_log: 7 günden eski görüntüleme loglarını sil (series_views_log gibi)
    try {
      db.prepare("DELETE FROM chapter_views_log WHERE created_at < datetime('now', '-7 days')").run();
    } catch {}

    // user_activity_log: 30 günden eski aktivite loglarını sil
    try {
      db.prepare("DELETE FROM user_activity_log WHERE created_at < datetime('now', '-30 days')").run();
    } catch {}

    // site_traffic_log: 7 günden eski trafik loglarını sil
    try {
      db.prepare("DELETE FROM site_traffic_log WHERE created_at < datetime('now', '-7 days')").run();
    } catch {}

    // ── Aşama 4: SQLite WAL checkpoint (WAL dosyası şişmesini önle) ────────
    try {
...
function initializeDatabase(db) {
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_chapters_series_publish ON chapters(series_id, publish_at, chapter_number DESC);
