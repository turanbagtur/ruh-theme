...
/**
 * Clean a chapter title: remove trailing dates, "Read", "Chapter X" prefixes if redundant,
...
function cleanChapterTitle(rawText, chapterNum) {
    ...
    let t = rawText
        ...
        // Remove "Read" prefixes like "Read Chapter 56", "Read First Chap"
        .replace(/^read\s+/i, '')
...
function buildChapterListFromAnchors($, seriesUrl) {
    ...
    if (chapterMap.size === 0) {
        $('a[href]').each((_, el) => {
            ...
            // Reject navigation links: "Read First", "Read Latest", "First Chapter" etc.
...
export async function fetchHtmlChapterPages(chapterUrl) {
    ...
    // 3. All img tags fallback
    if (images.length === 0) {
        $('img').each((_, el) => {
            const src = $(el).attr('src') || $(el).attr('data-src') || $(el).attr('data-lazy-src') || '';
            if (isValidImageUrl(src) && !/logo|icon|avatar|banner|ad[-_]/i.test(src)) images.push(src.trim());
        });
        images = dedupeUrls(images);
    }
...
function extractImagesFromScripts($, baseUrl) {
    ...
    $('script').each((_, el) => {
        ...
        // Quoted image URLs (cluster-based)
        const found = [];
        for (const m of src.matchAll(/"(https?:\/\/[^"]+\.(?:jpg|jpeg|png|webp|gif)(?:\?[^"]+)?)"/g)) {
            if (!/logo|icon|banner/i.test(m[1])) found.push(m[1]);
        }
        if (found.length >= 5) {
            const domains = {};
            ...
            if (top && top[1] >= 5) found.filter(u => { try { return new URL(u).hostname === top[0]; } catch { return false; } }).forEach(u => images.push(u));
        }

        // Next.js __NEXT_DATA__ image extraction
