...
export default function HomePage() {
  ...
  // Memoize card component selection to avoid switch on every item
  const CardComponent = useMemo(() => {
    switch (appSettings.latest_updates_design) {
      case 'style1': return ClassicCard;
      case 'style2': return CosmicTearCard;
      case 'style3': return SkillTreeCard;
      case 'style4': return HoloUpdateCard;
      case 'style5': return CinematicCard;
      case 'style6': return GlassmorphismCard;
      default: return HoloUpdateCard;
    }
  }, [appSettings.latest_updates_design]);
  ...
  return (
    ...
    <div className="home-wrapper">
      ...
      {!loading && appSettings?.donation_enabled === '1' && (() => {
        ...
        // Manuel yüzdelik mod: donation_goal_pct ayarlanmışsa onu kullan, yoksa tutardan hesapla
        const manualPct = parseFloat(appSettings.donation_goal_pct || '0') || 0;
        const goalPct = manualPct > 0
          ? Math.min(100, Math.max(0, manualPct))
          : Math.min(100, Math.max(0, (goalCurrent / goalTarget) * 100));
        const isPctMode = manualPct > 0; // Manuel yüzde modu aktif mi
        const isComplete = goalPct >= 100;
        const hasAnyBtn = appSettings.paypal_url || appSettings.kofi_url || appSettings.kreosus_url;
        return (
        <section className="page-container page-section pt-0" style={{ paddingBottom: '10px' }}>
          <div className="donation-banner">
            {/* Dekoratif parlama küreleri */}
            <div className="donation-orb donation-orb-1" />
            <div className="donation-orb donation-orb-2" />

            {/* Üst satır: ikon + metin + butonlar */}
            <div className="donation-top-row">
              <div className="donation-left">
                ...
                <div className="donation-text-col">
                  <span className="donation-title">
                    {appSettings.donation_text || 'Bize Destek Olun!'}
                  </span>
                  {goalEnabled && (
                    <span className="donation-subtitle">
                      {!isPctMode && (
                        <>
                          <span className="donation-goal-amount-current">
                            {goalCurrency}{Number(goalCurrent).toLocaleString('tr-TR')}
                          </span>
                          <span style={{ opacity: 0.45 }}>/</span>
                          <span className="donation-goal-amount-target">
                            {goalCurrency}{Number(goalTarget).toLocaleString('tr-TR')}
                          </span>
                        </>
                      )}
                      {isComplete
                        ? <span className="donation-pct-badge complete">✓ Hedefe Ulaşıldı!</span>
                        : <span className="donation-pct-badge">%{Math.round(goalPct)}</span>
                      }
                    </span>
                  )}
                </div>
                ...
                <div className="donation-banner-actions">
                  {appSettings.paypal_url && (
                    <a href={appSettings.paypal_url} target="_blank" rel="noopener noreferrer" className="donation-btn paypal">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" stroke="none"><path d="M7.076 21.337H2.47a.641.641 0 0 1-.633-.74L4.944.901C5.026.382 5.474 0 5.998 0h7.46c2.57 0 4.578.543 5.69 1.81 1.01 1.15 1.304 2.42 1.012 4.287-.023.143-.047.288-.077.437-.983 5.05-4.349 6.797-8.647 6.797h-2.19c-.524 0-.968.382-1.05.9l-1.12 7.106zm14.146-14.42a3.35 3.35 0 0 0-.607-.541c-.013.076-.026.175-.041.254-.59 3.025-2.566 6.082-8.558 6.082H9.825l-.173 1.089 1.158 7.346H7.28l-.96 6.081c-.065.412.253.787.67.787h3.918c.46 0 .85-.334.922-.787l.038-.196.733-4.645.047-.257c.072-.453.462-.787.922-.787h.58c3.76 0 6.7-1.528 7.558-5.946.36-1.847.174-3.39-.486-4.48z"/></svg>
                      PayPal
                    </a>
                  )}
                  {appSettings.kofi_url && (
                    <a href={appSettings.kofi_url} target="_blank" rel="noopener noreferrer" className="donation-btn kofi">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" stroke="none"><path d="M23.881 8.948c-.773-4.085-4.859-4.593-4.859-4.593H.723c-.604 0-.679.798-.679.798s-.082 7.324-.022 11.822c.164 2.424 2.586 2.672 2.586 2.672s8.267-.023 11.966-.049c2.438-.426 2.683-2.566 2.658-3.734 4.352.24 7.422-2.831 6.649-6.916zm-11.062 3.511c-1.246 1.453-4.011 3.976-4.011 3.976s-.121.119-.31.023c-.076-.057-.108-.09-.108-.09-.443-.441-3.368-3.049-4.034-3.954-.709-.965-1.041-2.7-.091-3.71.951-1.01 3.005-1.086 4.363.407 0 0 1.565-1.782 3.468-.963 1.904.82 1.832 3.011.723 4.311zm6.173.478c-.928.116-1.682.028-1.682.028V7.284h1.77s1.971.551 1.971 2.638c0 1.913-.985 2.667-2.059 3.015z"/></svg>
                      Ko-fi
                    </a>
                  )}
                  {appSettings.kreosus_url && (
                    <a href={appSettings.kreosus_url} target="_blank" rel="noopener noreferrer" className="donation-btn kreosus">
            ...
            {/* Hedef progress barı */}
            {goalEnabled && (
              <div className="donation-goal-wrap">
                {/* Thumb — bar dışında konumlandırıldı (overflow:hidden bug'ı giderildi) */}
                <div className="donation-goal-thumb-wrap">
                  {goalPct > 0 && goalPct < 100 && (
                    <div className="donation-goal-thumb" style={{ left: `${goalPct}%` }} />
                  )}
                </div>
                <div className="donation-goal-bar">
                  <div
                    className={`donation-goal-fill${isComplete ? ' complete' : ''}`}
                    style={{ width: `${goalPct}%` }}
                  />
                </div>
                <div className="donation-goal-labels">
                  <span>{isPctMode ? '%0' : `0 ${goalCurrency}`}</span>
                  <span className="donation-goal-label-mid">{goalLabel}</span>
                  <span className="donation-goal-amount-target">
                    {isPctMode ? '%100' : `${Number(goalTarget).toLocaleString('tr-TR')} ${goalCurrency}`}
                  </span>
                </div>
              </div>
            )}
        ...
        <div className="main-layout">
          ...
          <div className="main-content">
            ...
            {loading ? (
              ...
              <div className="updates-list" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 360px), 1fr))', gap: '12px', minWidth: 0, width: '100%' }}>
                {filteredUpdates.map(s => {
                  const seriesSlug = s.slug || s.id;
                  return (
                    <div key={s.id} style={{ position: 'relative', height: '100%', minWidth: 0 }}>
                    ...
                    {!!s.is_adult && !user && (
                      <a href="/login" style={{
                        ...
                        background: 'rgba(0,0,0,0.55)', borderRadius: 10, textDecoration: 'none',
                        color: '#fff', fontSize: '0.78rem', fontWeight: 700, backdropFilter: 'blur(2px)',
                    ...
                    <div style={!!s.is_adult && !user ? { filter: 'blur(5px)', pointerEvents: 'none', userSelect: 'none', borderRadius: 10, overflow: 'hidden', height: '100%', minWidth: 0 } : { height: '100%', minWidth: 0 }}>
                    <CardComponent
                      href={`/seri/${seriesSlug}`}
                      coverUrl={s.cover_url || '/demo/cover1.jpg'}
                      title={s.title}
                      titleColor={appSettings.latest_updates_title_color || undefined}
                      cardBg={appSettings.latest_updates_card_bg || undefined}
                      cardBorder={appSettings.latest_updates_card_border || undefined}
                    >
