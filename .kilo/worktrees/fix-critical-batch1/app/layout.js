...
export async function generateMetadata() {
  ...
  const faviconUrl = settings.favicon_url?.trim()
    ? (settings.favicon_url.startsWith('http') ? `${settings.favicon_url}?v=${Date.now()}` : `${BASE_URL}${settings.favicon_url}?v=${Date.now()}`)
    : null;
...
export default async function RootLayout({ children }) {
  ...
  // Favicon ve logo için cache önleme timestamp'i
