...
useEffect(() => {
    ...
    function handleReportSubmit() {
        ...
        setReportCooldown(prev => ({ ...prev, [commentId]: Date.now() + 60000 }));
    }
                            ...
                            <div style={{position: 'relative'}} ref={openMenu === c.id ? menuRef : null}>
                                ...
                                {openMenu === c.id && (
                                    <div className="comment-menu" style={{ left: 0, right: 'auto', minWidth: 160 }}>
                                        ...
                                        {!c.is_deleted && !isOwner && (() => {
                                            const cooldownUntil = reportCooldown[c.id] || 0;
                                            const remaining = Math.max(0, Math.ceil((cooldownUntil - Date.now()) / 1000));
                                            return (
                                                <button
                                                    className="comment-menu-item danger"
                                                    disabled={remaining > 0}
                                                    onClick={() => { if (remaining > 0) return; setReportModal(c.id); setOpenMenu(null); }}
                                                    style={remaining > 0 ? { opacity: 0.5, cursor: 'not-allowed' } : undefined}
                                                >
                                                    <FlagIcon /> {remaining > 0 ? `Bildir (${remaining}s)` : (appSettings.lang_report || 'Bildir')}
                                                </button>
                                            );
                                        })()}
                                    </div>
