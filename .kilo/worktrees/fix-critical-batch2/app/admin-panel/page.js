    ...
    async function handleSiteAssetUpload(e, assetType, urlKey) {
        ...
        try {
            ...
            if (data.success && data.path) {
                ...
                alert('Görsel yüklendi: ' + data.path);
            } else {
...
// Bölüm başı görseli ayarlarını yükle
    ...
    async function handleAddChapter() {
        ...
        setSubmitting(true);
        try {
            const r1 = await doAction('add-chapter', { seriesId: detailSeries.id, chapterNumber: cNum, title: cTitle || `Bölüm ${cNum}`, content: cContent, publishAt: cPublishAt || null });
            const newChapId = r1.chapterId;
            ...
            setCNum(''); setCTitle(''); setCFiles(null); setCContent(''); setCPublishAt('');
                        ...
                        {viewingUser && (
                            <div className="admin-card" style={{ marginBottom: 20, border: '1px solid var(--accent)' }}>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 10, marginBottom: 14 }}>
                                    {[
                                        { label: 'Rol', value: { admin: 'Kurucu', manager: 'Yönetici', moderator: 'Moderatör', team_member: 'Yükleyici', user: 'Kullanıcı' }[viewingUser.user?.role] || viewingUser.user?.role },
                                    ...
                                    {(() => {
                                        ...
                                        <tr key={u.id}>
                                            ...
                                            <td><span className={`admin-badge ${u.role !== 'user' ? 'admin-role' : 'user-role'}`}>{
                                                {admin: 'Kurucu', manager: 'Yönetici', moderator: 'Moderatör', team_member: 'Yükleyici', user: 'Kullanıcı'}[u.role] || 'Kullanıcı'
                                            }</span></td>
                    ...
                    <>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 10 }}>
                            <h2 style={{ fontSize: '1.2rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8, margin: 0 }}>
                                <MsgIcon /> Yorumlar
                                <span style={{ fontSize: '0.75rem', fontWeight: 400, color: 'var(--text-muted)', background: 'var(--bg-tertiary)', padding: '2px 8px', borderRadius: 20 }}>
                                    {stats?.totalComments || 0} toplam
                                </span>
                            </h2>
                        </div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12, marginBottom: 20 }}>
                            {[
                                { label: 'Toplam Yorum', value: stats?.totalComments || 0, color: '#818cf8', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#818cf8" strokeWidth="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg> },
                                { label: 'Son 24 Saat', value: (stats?.recentComments || []).filter(c => { try { return (Date.now() - new Date(c.created_at).getTime()) < 86400000; } catch { return false; } }).length, color: '#4ade80', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#4ade80" strokeWidth="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg> },
                                { label: 'Son 7 Gün', value: (stats?.recentComments || []).filter(c => { try { return (Date.now() - new Date(c.created_at).getTime()) < 604800000; } catch { return false; } }).length, color: '#fbbf24', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fbbf24" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg> },
                                { label: 'Gösterilen', value: (() => { const all = stats?.recentComments || []; const q = commentSearch.trim().toLowerCase(); const filtered = all.filter(c => { const matchQ = !q || c.content?.toLowerCase().includes(q) || c.username?.toLowerCase().includes(q) || c.series_title?.toLowerCase().includes(q); const matchF = commentFilter === 'all' || (commentFilter === 'series' && !c.chapter_id) || (commentFilter === 'chapter' && !!c.chapter_id); return matchQ && matchF; }); return filtered.length; })(), color: '#f472b6', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#f472b6" strokeWidth="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg> },
                            ].map(s => (
                                <div key={s.label} style={{ background: 'var(--bg-card)', borderRadius: 10, padding: '14px 16px', border: `1px solid ${s.color}33`, display: 'flex', alignItems: 'center', gap: 12 }}>
                                    <div style={{ width: 36, height: 36, borderRadius: 8, background: `${s.color}18`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{s.icon}</div>
                                    <div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                                ...
                                <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
                                    <div style={{ position: 'relative', flex: 1, minWidth: 200 }}>
                                        ...
                                        <input
                                            type="text"
                                            className="form-input"
                                            placeholder="Kullanıcı, yorum veya seri ara..."
                                    ...
                                    <select
                                        value={commentFilter}
                                        onChange={e => { setCommentFilter(e.target.value); setCommentsPage(1); }}
                                        style={{ background: 'var(--bg-tertiary)', border: '1px solid var(--border-color)', color: 'var(--text-primary)', borderRadius: 6, padding: '6px 10px', fontSize: '0.82rem', minWidth: 140 }}
                                    >
                                        <option value="all">Tüm Yorumlar</option>
                                        <option value="series">Seri Yorumları</option>
                                        <option value="chapter">Bölüm Yorumları</option>
                                    </select>
                                    {(commentSearch || commentFilter !== 'all') && (
                                        <button className="btn btn-ghost btn-sm" onClick={() => { setCommentSearch(''); setCommentFilter('all'); setCommentsPage(1); }} style={{ whiteSpace: 'nowrap' }}>
                                            ✕ Temizle
                                        </button>
                                    )}
                                </div>
                                ...
                                {(() => {
                                    ...
                                    if (filtered.length === 0) {
                                        return (
                                            <div className="admin-card" style={{ textAlign: 'center', padding: '40px 20px' }}>
                                                <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" style={{ opacity: 0.2, display: 'block', margin: '0 auto 12px' }}><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
                                                <p style={{ color: 'var(--text-muted)', margin: 0, fontSize: '0.88rem' }}>
                                                    {commentSearch || commentFilter !== 'all' ? 'Aramanızla eşleşen yorum bulunamadı.' : 'Henüz yorum yapılmamış.'}
                                                </p>
                                            </div>
                                        );
                                    }
                                    ...
                                    return (
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                            {paginated.map(c => {
                                                const commentUrl = c.series_slug
                                                    ? (c.chapter_id && c.chapter_number != null)
                                                        ? `/seri/${c.series_slug}/bolum/${c.chapter_number}`
                                                        : `/seri/${c.series_slug}`
                                                    : null;
                                                const initials = (c.username || '?').slice(0, 2).toUpperCase();
                                                const avatarColors = ['#818cf8', '#4ade80', '#f472b6', '#fbbf24', '#60a5fa', '#fb923c'];
                                                const avatarColor = avatarColors[(c.username || '').charCodeAt(0) % avatarColors.length];
                                                const isChapter = !!c.chapter_id;
                                                const typeLabel = isChapter ? 'Bölüm' : 'Seri';
                                                const typeColor = isChapter ? '#60a5fa' : '#818cf8';
                                                return (
                                                    <div key={c.id} style={{ background: 'var(--bg-card)', borderRadius: 10, border: '1px solid var(--border-color)', padding: '14px 16px', transition: 'border-color 0.15s' }}
                                                        onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--accent)'}
                                                        onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--border-color)'}
                                                    >
                                                        {/* Top row: avatar + user + meta + actions */}
                                                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                                                            {/* Avatar */}
                                                            <div style={{ width: 36, height: 36, borderRadius: '50%', background: `${avatarColor}28`, border: `2px solid ${avatarColor}55`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.75rem', fontWeight: 700, color: avatarColor, flexShrink: 0 }}>
                                                                {initials}
                                                            </div>
                                                            {/* User + meta */}
                                                            <div style={{ flex: 1, minWidth: 0 }}>
                                                                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 6 }}>
                                                                    <span style={{ fontWeight: 700, fontSize: '0.88rem' }}>{c.username || 'Bilinmeyen'}</span>
                                                                    <span style={{ fontSize: '0.68rem', padding: '1px 7px', borderRadius: 20, background: `${typeColor}20`, color: typeColor, fontWeight: 600, border: `1px solid ${typeColor}40` }}>{typeLabel}</span>
                                                                    <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginLeft: 'auto' }}>{fDate(c.created_at)}</span>
                                                                </div>
                                                                {/* Comment content */}
                                                                <p style={{ margin: '0 0 8px', fontSize: '0.86rem', color: 'var(--text-secondary)', lineHeight: 1.5, wordBreak: 'break-word', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                                                                    {c.content}
                                                                </p>
                                                                {/* Source link */}
                                                                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                                                                    {c.series_title && (
                                                                        <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.75rem', color: 'var(--text-muted)', background: 'var(--bg-tertiary)', padding: '3px 8px', borderRadius: 5, overflow: 'hidden', maxWidth: '100%' }}>
                                                                            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ flexShrink: 0 }}><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>
                                                                            <span style={{ fontWeight: 600, color: 'var(--accent-light)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.series_title}</span>
                                                                            {c.chapter_title && c.chapter_title !== 'Series Comment' && (
                                                                                <><span style={{ opacity: 0.4 }}>/</span><span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.chapter_title}</span></>
                                                                            )}
                                                                        </div>
                                                                    )}
                                                                    {/* ID badge */}
                                                                    <span style={{ fontSize: '0.68rem', color: 'var(--text-muted)', opacity: 0.5 }}>#{c.id}</span>
                                                                </div>
                                                            </div>
                                                            {/* Action buttons */}
                                                            <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                                                                {commentUrl && (
                                                                    <a href={commentUrl} target="_blank" rel="noopener noreferrer" className="btn btn-ghost btn-sm" title="Yoruma git" style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.75rem' }}>
                                                                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
                                                                        Git
                                                                    </a>
                                                                )}
                                                                <button className="btn btn-danger btn-sm" title="Yorumu sil" onClick={() => setConfirmModal({ action: 'delete-comment', body: { commentId: c.id }, text: 'Bu yorumu silmek istediğinize emin misiniz?' })} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.75rem' }}>
                                                                    <TrashIcon /> Sil
                                                                </button>
                                                            </div>
                                                        </div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                                ...
                                <div className="admin-card" style={{ margin: 0 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                                        <div style={{ width: 28, height: 28, borderRadius: 7, background: 'rgba(251,191,36,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1rem' }}>🎯</div>
                                        <h3 style={{ margin: 0, fontSize: '0.95rem', fontWeight: 700 }}>Tepki Emojileri</h3>
                                    </div>
                                    <p style={{ color: 'var(--text-muted)', fontSize: '0.76rem', marginBottom: 12, marginTop: 6 }}>
                                        Yorumlarda gösterilecek tepki emojileri ve etiketleri.
                                    </p>

                                    {/* Emoji preview strip */}
                                    {commentEmojis.length > 0 && (
                                        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12, padding: '8px 10px', background: 'var(--bg-tertiary)', borderRadius: 7 }}>
                                            {commentEmojis.map((em, i) => (
                                                <span key={i} style={{ fontSize: '0.78rem', padding: '2px 8px', background: 'var(--bg-card)', borderRadius: 20, border: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: 4 }}>
                                                    <span style={{ fontSize: '1rem' }}>{em.icon}</span> {em.label}
                                                </span>
                                            ))}
                                        </div>
                                    )}
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 10 }}>
                                        {commentEmojis.map((em, index) => (
                                            <div key={index} style={{ display: 'flex', gap: 6, alignItems: 'center', padding: '4px 8px', background: 'var(--bg-tertiary)', borderRadius: 7, border: '1px solid var(--border-color)' }}>
                                                <input
                                                    type="text"
                                                    className="form-input"
                                                    value={em.icon}
                                                    onChange={e => updateEmojiField(index, 'icon', e.target.value)}
                                                    style={{ width: 44, textAlign: 'center', padding: '3px 5px', fontSize: '1.1rem', background: 'transparent', border: 'none', outline: 'none' }}
                                                    placeholder="👍"
                                                />
                                                <input
                                                    type="text"
                                                    ...
                                                    style={{ flex: 1, padding: '3px 7px', fontSize: '0.82rem', background: 'transparent', border: 'none', outline: 'none' }}
                                                    placeholder="Etiket"
                                                />
                                                <button className="btn btn-ghost btn-sm" onClick={() => removeEmoji(index)} style={{ padding: '4px 6px', color: 'var(--danger, #ef4444)', opacity: 0.7 }} title="Kaldır">
                                                    <TrashIcon />
                                                </button>
                                            </div>
                                        ))}
                                    ...
                                    <div style={{ padding: '8px 10px', background: 'var(--bg-tertiary)', borderRadius: 7, marginBottom: 10, border: '1px dashed var(--border-color)' }}>
                                        <div style={{ fontSize: '0.73rem', fontWeight: 600, color: 'var(--text-muted)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Yeni Emoji Ekle</div>
                                        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                                            <input
                                                type="text"
                                                className="form-input"
                                                ...
                                                style={{ width: 44, textAlign: 'center', padding: '4px 6px', fontSize: '1.1rem' }}
                                                placeholder="👍"
                                            ...
                                            <input
                                                ...
                                                onKeyDown={e => e.key === 'Enter' && addEmoji()}
                                            />
                                            <button className="btn btn-primary btn-sm" onClick={addEmoji} disabled={!newEmojiIcon.trim() || !newEmojiLabel.trim()}>Ekle</button>
                                        </div>
                                    </div>
                                    <button
                                ...
                                <div className="admin-card" style={{ margin: 0 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                                        <div style={{ width: 28, height: 28, borderRadius: 7, background: 'rgba(239,68,68,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1rem' }}>🚩</div>
                                        <h3 style={{ margin: 0, fontSize: '0.95rem', fontWeight: 700 }}>Bildirim Nedenleri</h3>
                                    </div>
                                    <p style={{ color: 'var(--text-muted)', fontSize: '0.76rem', marginBottom: 12, marginTop: 6 }}>
                                        Raporlamada kullanıcılara sunulan nedenler.
                                    </p>

                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 10 }}>
                                        {reportReasons.map((re, index) => (
                                            <div key={index} style={{ display: 'flex', gap: 6, alignItems: 'center', padding: '4px 8px', background: 'var(--bg-tertiary)', borderRadius: 7, border: '1px solid var(--border-color)' }}>
                                                <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-muted)', minWidth: 18, textAlign: 'right' }}>{index + 1}.</span>
                                                <input
                                                    type="text"
                                                    className="form-input"
                                                    value={re}
                                                    ...
                                                    style={{ flex: 1, padding: '3px 7px', fontSize: '0.82rem', background: 'transparent', border: 'none', outline: 'none' }}
                                                />
                                                <button className="btn btn-ghost btn-sm" onClick={() => removeReason(index)} style={{ padding: '4px 6px', color: 'var(--danger, #ef4444)', opacity: 0.7 }} title="Kaldır">
                                                    <TrashIcon />
                                                </button>
                                            </div>
                                        ))}
                                    ...
                                    <div style={{ padding: '8px 10px', background: 'var(--bg-tertiary)', borderRadius: 7, marginBottom: 10, border: '1px dashed var(--border-color)' }}>
                                        <div style={{ fontSize: '0.73rem', fontWeight: 600, color: 'var(--text-muted)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Yeni Neden Ekle</div>
                                        <div style={{ display: 'flex', gap: 6 }}>
                                            <input
                                                type="text"
                                                className="form-input"
                                            ...
                                            />
                                            <button className="btn btn-primary btn-sm" onClick={addReason} disabled={!newReasonText.trim()}>Ekle</button>
                                        </div>
                                    </div>
                                    <button
                                        className="btn btn-primary"
                                        ...
                                        style={{ width: '100%', fontSize: '0.85rem' }}
                                    >
                                        {submitting ? 'Kaydediliyor...' : '💾 Nedenleri Kaydet'}
                                    </button>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <tbody>
                                        {bugReports.slice((reportsPage - 1) * ADMIN_PAGE_SIZE, reportsPage * ADMIN_PAGE_SIZE).map(rep => (
                                            <tr key={rep.id}>
                                                <td>
                                                    <span style={{
                                                        fontSize: '0.7rem', padding: '2px 8px', borderRadius: 50, fontWeight: 700,
                                                        background: rep.type === 'comment' ? 'rgba(99,102,241,0.15)' : 'rgba(245,158,11,0.12)',
                                                        color: rep.type === 'comment' ? '#818cf8' : '#f59e0b',
                                                        whiteSpace: 'nowrap',
                                                    }}>
                                                        {rep.type === 'comment' ? '💬 Yorum' : '🐛 Hata'}
                                                    </span>
                                                </td>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 14 }}>
                                    <div className="form-group" style={{ margin: 0 }}>
                                        ...
                                        <select className="form-input" value={customize.latest_updates_design || 'style4'} onChange={e => setCustomize({ ...customize, latest_updates_design: e.target.value })}>
                                            ...
                                            <option value="style6">6. Glassmorphism (Cam Efekti)</option>
                                        </select>
