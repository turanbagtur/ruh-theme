...
const { settings: siteSettings } = useSettings() || {};
    ...
    const [librarySubTab, setLibrarySubTab] = useState('favorites'); // 'favorites' | 'reading-list'
    const [settingsSubTab, setSettingsSubTab] = useState('profile'); // 'profile' | 'password' | 'privacy'
    ...
    return (
        <div className="page-container page-section fade-in">
            ...
            <div className="profile-stats-row" style={{ marginTop: '24px' }}>
                ...
                <div className="profile-stat-card" style={{ cursor: 'pointer' }} onClick={() => { setTab('favorites'); setLibrarySubTab('reading-list'); }} title="Okuma Listesine Git">
            ...
            {/* Library/Favorites + Reading List (birleşik) */}
            {tab === 'favorites' && (
                <div>
                    {/* Sub-tab toggle */}
                    <div className="reading-list-tabs" style={{ marginBottom: 20 }}>
                        <button
                            className={`reading-list-tab ${librarySubTab === 'favorites' ? 'active' : ''}`}
                        ...
                        <button
                            className={`reading-list-tab ${librarySubTab === 'reading-list' ? 'active' : ''}`}
                            onClick={() => setLibrarySubTab('reading-list')}
                            style={{ display: 'flex', alignItems: 'center', gap: 6 }}
                        >
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>
                            Okuma Listesi ({readingList.length})
                        </button>
                    </div>
                    ...
                    {/* Okuma Listesi */}
                    {librarySubTab === 'reading-list' && (
                        <div>
                            <div className="reading-list-tabs" style={{ marginBottom: 16 }}>
                                {[
            ...
            {/* Settings (Profil Bilgileri + Şifre Değiştir + Gizlilik) */}
            {tab === 'settings' && (
                <div>
                    {/* Settings sub-tab navigation */}
                    <div className="reading-list-tabs" style={{ marginBottom: 20 }}>
                        <button
                            className={`reading-list-tab ${settingsSubTab === 'profile' ? 'active' : ''}`}
                        ...
                        <button
                            className={`reading-list-tab ${settingsSubTab === 'password' ? 'active' : ''}`}
                        ...
                        <button
                            className={`reading-list-tab ${settingsSubTab === 'privacy' ? 'active' : ''}`}
                    ...
                    {settingsSubTab === 'privacy' && (
                        <div className="admin-card" style={{ maxWidth: 500 }}>
                            ...
                            <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 16 }}>
                                <div style={{ padding: '14px 16px', background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border-color)', borderRadius: 10 }}>
                                    <div style={{ fontWeight: 700, fontSize: '0.9rem', marginBottom: 6 }}>Hesap Güvenliği</div>
                                    <p style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', marginBottom: 12 }}>
                                        Hesabınızın güvende olduğundan emin olun. Şifrenizi düzenli olarak değiştirmenizi öneririz.
                                    </p>
                                    <button className="btn btn-ghost btn-sm" onClick={() => setSettingsSubTab('password')}>
                                        Şifre Değiştir →
                                    </button>
                                </div>
                                <div style={{ padding: '14px 16px', background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border-color)', borderRadius: 10 }}>
                                    <div style={{ fontWeight: 700, fontSize: '0.9rem', marginBottom: 6 }}>Profil Bilgileri</div>
                                    ...
                                    <button className="btn btn-ghost btn-sm" onClick={() => setSettingsSubTab('profile')}>
                                        Profili Düzenle →
                                    </button>
                                </div>
                                <div style={{ padding: '14px 16px', background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 10 }}>
                                    <div style={{ fontWeight: 700, fontSize: '0.9rem', marginBottom: 6, color: '#f87171' }}>Oturumu Kapat</div>
                                    <p style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', marginBottom: 12 }}>
