...
function StarRating({ rating, maxStars = 5 }) {
    ...
    for (let i = 1; i <= maxStars; i++) {
        ...
        const gradId = `sf-${uid}-${i}-${Math.round(rating * 10)}`;
        stars.push(
    ...
    return <div style={{ display: 'flex', gap: '1px', alignItems: 'center' }}>{stars}</div>;
}

function RatingBadge({ rating, voteCount }) {
    if (rating == null) {
        return (
            <span style={{
    ...
    const isGreen  = r >= 4.0;
    const isYellow = r >= 3.0 && r < 4.0;
    const isOrange = r >= 2.0 && r < 3.0;

    const bg     = isGreen ? 'rgba(34,197,94,0.15)'  : isYellow ? 'rgba(245,158,11,0.15)'  : isOrange ? 'rgba(249,115,22,0.15)'  : 'rgba(239,68,68,0.15)';
    const border  = isGreen ? 'rgba(34,197,94,0.35)'  : isYellow ? 'rgba(245,158,11,0.35)'  : isOrange ? 'rgba(249,115,22,0.35)'  : 'rgba(239,68,68,0.35)';
    const color   = isGreen ? '#4ade80'               : isYellow ? '#fbbf24'                : isOrange ? '#fb923c'                : '#f87171';
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <div style={{
                ...
                <svg width="9" height="9" viewBox="0 0 24 24" style={{ flexShrink: 0 }}>
                    <polygon fill={color} stroke={color} strokeWidth="1" strokeLinejoin="round"
                        points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
                </svg>
                {r.toFixed(1)}
                {voteCount != null && voteCount > 0 && (
                    <span style={{ opacity: 0.6, fontWeight: 500, fontSize: '0.65rem' }}>({voteCount})</span>
                )}
            </div>
            <StarRating rating={r} />
        </div>
...
src={series.cover_url || DEFAULT_COVER}
            ...
            <div className="series-card-body">
                ...
                <div className="series-card-meta">
                    <RatingBadge rating={series.rating} voteCount={series.vote_count ?? series.rating_count} />
