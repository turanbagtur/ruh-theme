...
const { settings: siteSettings } = useSettings() || {};
    ...
    async function saveAvatar() {
        ...
        try {
            ...
            formData.append('cropX', avatarCropData.x / avatarCropData.scale);
            formData.append('cropY', avatarCropData.y / avatarCropData.scale);
    ...
    async function saveCover() {
        ...
        try {
            ...
            formData.append('cropX', coverCropData.x / coverCropData.scale);
            formData.append('cropY', coverCropData.y / coverCropData.scale);
                                ...
                                <div style={{ marginBottom: 12 }}>
                                    ...
                                    <div
                                        style={{
                                            ...
                                            border: '2px solid var(--accent)', background: '#111',
                                            cursor: avatarIsDragging ? 'grabbing' : 'grab',
                                            position: 'relative', userSelect: 'none', touchAction: 'none',
                                        }}
                                        onMouseDown={e => {
                                            e.preventDefault();
                                            setAvatarIsDragging(true);
                                            setAvatarDragStart({ x: e.clientX - avatarCropData.x, y: e.clientY - avatarCropData.y });
                                        }}
                                        onMouseMove={e => {
                                            if (!avatarIsDragging) return;
                                            const x = e.clientX - avatarDragStart.x;
                                            const y = e.clientY - avatarDragStart.y;
                                            if (avatarRafRef.current) cancelAnimationFrame(avatarRafRef.current);
                                            avatarRafRef.current = requestAnimationFrame(() => {
                                                setAvatarCropData(p => ({ ...p, x, y }));
                                            });
                                        }}
                                        onMouseUp={() => { cancelAnimationFrame(avatarRafRef.current); setAvatarIsDragging(false); }}
                                        onMouseLeave={() => { cancelAnimationFrame(avatarRafRef.current); setAvatarIsDragging(false); }}
                                        onTouchStart={e => {
                                            const t = e.touches[0];
                                            setAvatarIsDragging(true);
                                            setAvatarDragStart({ x: t.clientX - avatarCropData.x, y: t.clientY - avatarCropData.y });
                                        }}
                                        onTouchMove={e => {
                                            if (!avatarIsDragging) return;
                                            const t = e.touches[0];
                                            const x = t.clientX - avatarDragStart.x;
                                            const y = t.clientY - avatarDragStart.y;
                                            if (avatarRafRef.current) cancelAnimationFrame(avatarRafRef.current);
                                            avatarRafRef.current = requestAnimationFrame(() => {
                                                setAvatarCropData(p => ({ ...p, x, y }));
                                            });
                                        }}
                                        onTouchEnd={() => { cancelAnimationFrame(avatarRafRef.current); setAvatarIsDragging(false); }}
                                    >
                                        <img
                                            src={avatarPreview}
                                            ...
                                            style={{
                                                ...
                                                transform: `scale(${avatarCropData.scale}) translate(${avatarCropData.x / avatarCropData.scale}px, ${avatarCropData.y / avatarCropData.scale}px)`,
                                                transformOrigin: 'center center',
                                                transition: avatarIsDragging ? 'none' : 'transform 0.1s',
                                            }}
                                        />
                                        ...
                                        <div style={{ position: 'absolute', bottom: 8, left: 0, right: 0, textAlign: 'center', fontSize: '0.7rem', color: 'rgba(255,255,255,0.7)', pointerEvents: 'none' }}>
                                            Sürükleyerek konumlandırın
                                        </div>
                                    </div>

                                    {/* Zoom slider + preview + reset */}
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 10 }}>
                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--text-muted)" strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/><line x1="8" y1="11" x2="14" y2="11"/></svg>
                                        <input
                                            type="range" min="0.5" max="3" step="0.05"
                                            value={avatarCropData.scale}
                                            onChange={e => setAvatarCropData(p => ({ ...p, scale: parseFloat(e.target.value) }))}
                                            style={{ flex: 1, accentColor: 'var(--accent)' }}
                                        />
                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--text-muted)" strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/><line x1="11" y1="8" x2="11" y2="14"/><line x1="8" y1="11" x2="14" y2="11"/></svg>
                                        ...
                                        <div style={{
                                            ...
                                            <img src={avatarPreview} draggable={false} style={{
                                                width: '100%', height: '100%', objectFit: 'cover', pointerEvents: 'none',
                                                transform: `scale(${avatarCropData.scale}) translate(${avatarCropData.x / avatarCropData.scale}px, ${avatarCropData.y / avatarCropData.scale}px)`,
                                                transformOrigin: 'center center',
                                            }} />
                                        ...
                                        <button type="button" onClick={() => setAvatarCropData({ x: 0, y: 0, scale: 1 })}
                                            style={{ background: 'var(--bg-tertiary)', border: '1px solid var(--border)', borderRadius: 6, padding: '4px 10px', cursor: 'pointer', color: 'var(--text-muted)', fontSize: '0.72rem', flexShrink: 0 }}>
                                            Sıfırla
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div style={{ marginBottom: 12 }}>
                                    ...
                                    <div
                                        style={{
                                            ...
                                            border: '2px solid var(--accent)', background: '#111',
                                            cursor: coverIsDragging ? 'grabbing' : 'grab',
                                            position: 'relative', userSelect: 'none', touchAction: 'none',
                                        }}
                                        onMouseDown={e => {
                                            e.preventDefault();
                                            setCoverIsDragging(true);
                                            setCoverDragStart({ x: e.clientX - coverCropData.x, y: e.clientY - coverCropData.y });
                                        }}
                                        onMouseMove={e => {
                                            if (!coverIsDragging) return;
                                            const x = e.clientX - coverDragStart.x;
                                            const y = e.clientY - coverDragStart.y;
                                            if (coverRafRef.current) cancelAnimationFrame(coverRafRef.current);
                                            coverRafRef.current = requestAnimationFrame(() => {
                                                setCoverCropData(p => ({ ...p, x, y }));
                                            });
                                        }}
                                        onMouseUp={() => { cancelAnimationFrame(coverRafRef.current); setCoverIsDragging(false); }}
                                        onMouseLeave={() => { cancelAnimationFrame(coverRafRef.current); setCoverIsDragging(false); }}
                                        onTouchStart={e => {
                                            const t = e.touches[0];
                                            setCoverIsDragging(true);
                                            setCoverDragStart({ x: t.clientX - coverCropData.x, y: t.clientY - coverCropData.y });
                                        }}
                                        onTouchMove={e => {
                                            if (!coverIsDragging) return;
                                            const t = e.touches[0];
                                            const x = t.clientX - coverDragStart.x;
                                            const y = t.clientY - coverDragStart.y;
                                            if (coverRafRef.current) cancelAnimationFrame(coverRafRef.current);
                                            coverRafRef.current = requestAnimationFrame(() => {
                                                setCoverCropData(p => ({ ...p, x, y }));
                                            });
                                        }}
                                        onTouchEnd={() => { cancelAnimationFrame(coverRafRef.current); setCoverIsDragging(false); }}
                                    >
                                        <img
                                            src={coverPreview}
                                            alt="Kapak Kırpma Önizlemesi"
                                            draggable={false}
                                            style={{
                                                width: '100%', height: '100%', objectFit: 'cover', pointerEvents: 'none',
                                                transform: `scale(${coverCropData.scale}) translate(${coverCropData.x / coverCropData.scale}px, ${coverCropData.y / coverCropData.scale}px)`,
                                                transformOrigin: 'center center',
                                                transition: coverIsDragging ? 'none' : 'transform 0.1s',
                                            }}
                                        />
                                        ...
                                        <div style={{ position: 'absolute', bottom: 8, right: 10, fontSize: '0.65rem', color: 'rgba(255,255,255,0.5)', background: 'rgba(0,0,0,0.5)', padding: '2px 6px', borderRadius: 4, pointerEvents: 'none' }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 10 }}>
                                        ...
                                        <input
                                            type="range" min="0.5" max="3" step="0.05"
                                            value={coverCropData.scale}
                                            onChange={e => setCoverCropData(p => ({ ...p, scale: parseFloat(e.target.value) }))}
                                            style={{ flex: 1, accentColor: 'var(--accent)' }}
                                        />
                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--text-muted)" strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/><line x1="11" y1="8" x2="11" y2="14"/><line x1="8" y1="11" x2="14" y2="11"/></svg>
                                        ...
                                        {/* Preview thumbnail */}
                                        <div style={{
                                            width: 64, height: 36, borderRadius: 6, overflow: 'hidden', flexShrink: 0,
                                            border: '2px solid var(--accent)', background: '#111',
                                        }}>
                                            <img src={coverPreview} draggable={false} style={{
                                                width: '100%', height: '100%', objectFit: 'cover', pointerEvents: 'none',
                                                transform: `scale(${coverCropData.scale}) translate(${coverCropData.x / coverCropData.scale}px, ${coverCropData.y / coverCropData.scale}px)`,
                                                transformOrigin: 'center center',
                                            }} />
                                        ...
                                        <button type="button" onClick={() => setCoverCropData({ x: 0, y: 0, scale: 1 })}
                                            style={{ background: 'var(--bg-tertiary)', border: '1px solid var(--border)', borderRadius: 6, padding: '4px 10px', cursor: 'pointer', color: 'var(--text-muted)', fontSize: '0.72rem', flexShrink: 0 }}>
                                            Sıfırla
                                        </button>
                                    </div>
